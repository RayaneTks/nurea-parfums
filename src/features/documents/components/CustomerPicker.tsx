...
/**
 * S06 — Sélecteur de client depuis la fiche document (« Lier une fiche ») : récents (8 clients au document le plus
 * récent), recherche à la frappe (nom, téléphone normalisé, Snap, WhatsApp), et S10 — création en ligne avec
 * alerte d'homonyme. Sans la rangée « Client de passage » (06 S06).
 */
export function CustomerPicker({ open, onOpenChange, onSelect, nested = true }: CustomerPickerProps) {
  ...
  const options = useMemo<SelectOption[]>(() => {
    const hits = results ? (results.q.length >= 2 ? results.customers.items : results.recentCustomers) : [];
    return hits.map((hit) => ({
      value: hit.id,
      label: hit.fullName,
  ...
  return (
    <SelectSheet
      ...
      onCreate={{
        ...
        form: (ctx) => <CustomerCreateForm ctx={ctx} options={options} />,
      }}
    />
  );
}
