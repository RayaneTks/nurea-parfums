...
export function TicketBatchPicker({
  saleId,
  current,
  onAssigned,
  onError,
}: TicketBatchPickerProps) {
  ...
  const assign = async (batchId: string | null) => {
    setSaving(true);
    try {
      const r = await fetch(`/api/admin/sales/${saleId}`, {
        method: "PATCH",
        credentials: "include",
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            <span className="truncate">
              {current ? current.name : "Aucun lot — assigner"}
            </span>
            <ChevronDown
              size={14}
              ...
              {batches.map((b) => {
                ...
                return (
                  <li key={b.id}>
                    ...
                    >
                      <span className="truncate text-[14px] text-[var(--admin-text)]">
                        {b.name}
                      </span>
                      {active ? (
