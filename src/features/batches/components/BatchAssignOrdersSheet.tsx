...
export function BatchAssignOrdersSheet({ batchId, open, onOpenChange, onSaved, onError }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setQuery("");
    fetch(`/api/admin/batches/${batchId}/order-candidates`, {
      credentials: "include",
      cache: "no-store",
    })
      .then(async (r) => {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const r = await fetch(`/api/admin/batches/${batchId}/assign-orders`, {
        method: "POST",
        credentials: "include",
      ...
      <>
        ...
        <Stack gap={3} className="pt-3">
          {loading ? (
            ...
            <EmptyState
              icon={ListChecks}
              title={q.length > 0 ? "Aucun résultat" : "Aucune commande disponible"}
              description={
                q.length > 0
                  ? "Essaie un autre nom de client."
                  : "Seules les commandes « À traiter » ou « Livrée » non vendues sont éligibles."
              }
            />
          ) : (
            <ul className="space-y-1.5">
              {filtered.map((c) => {
                ...
                const date = new Date(c.orderedAt).toLocaleDateString("fr-FR", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                });
                  ...
                  <li key={c.id}>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <Money value={c.total} bold className="text-[13px]" />
                      </div>
                    </button>
                  </li>
                );
