...
- Si un outil ajoute automatiquement un trailer IA, corriger le commit avant push (historique propre obligatoire).
...
## Méthode qualité (audit -> correctif)

- Pour une demande "revue complete", proceder en sequence: audit des frictions -> priorisation severite -> implementation -> verification technique (`npm run lint`, `npx tsc --noEmit`).
- Mobile d'abord: verifier navigation attendue (retour haut gauche), champs effaçables rapidement (croix), actions principales lisibles et accessibles au pouce.
- Favoriser les patterns reutilisables pour garder une ergonomie coherente entre onglets/admin pages.
