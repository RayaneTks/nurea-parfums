...
import type { DocumentSetDTO, MargeNetteDTO, TresorerieDTO } from "../../src/contracts/chiffres";
 ...
 * (04 §16.4).
 */
export function chiffresCanoniques(periode: string): ChiffresCanoniques {
  if (E2E_REMOTE) throw new Error("Chiffres canoniques : jamais en mode distant.");
  const root = process.cwd();
  const output = execFileSync(
    process.execPath,
    [path.join(root, "node_modules", "tsx", "dist", "cli.mjs"), path.join("e2e", "support", "chiffres-canoniques.ts"), JSON.stringify({ periode })],
...
/** « 1 240,00 € » tel que l'écran l'écrit (`Money` : espace fine insécable). */
export const shown = (value: MoneyString, options?: { signed?: boolean }) => formatEur(eurFromWire(value), options);
