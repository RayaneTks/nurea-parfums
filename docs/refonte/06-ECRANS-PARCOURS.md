...
| E15 | `/admin/catalogue` | `(gestion)/catalogue/page.tsx` | Catalogue | — (racine) | `tab=parfums\|marques\|en-avant` (défaut `parfums`), `q`, `stock=bas\|rupture`, `visibilite=visibles|masques` | onglet ; alerte stock de E01 |
...
| Client avec fiche | 4 + saisie | ≈ 8 s | Champ « Nom du client » : taper le nom (la fiche du même nom est reprise), ou toucher la suggestion |
...
- **Zones.**
  0. **« À rattacher »** (écart du 17/09/2026 : capacité de production `521e086`, **avant les listes de lots** — ce qui reste à ranger, personne ne pense à aller le chercher ; absente s'il n'y a rien) : `SearchField` (visible au-delà de 6 lignes ou tant qu'il filtre ; mêmes champs que E10 zone 3, debounce 250 ms) ; `ListSection` « À rattacher · 17 » ; lignes = **documents non annulés sans lot**, commandes **et** ventes, **livrés compris** (un envoi terminé se rattache pour lui imputer transport et douane), du plus récent au plus ancien ; ligne : client, légende « Commande du 12 sept. · Sauvage, Libre +2 · En attente / Livrée » suivie du montant (« À encaisser » en `warning` s'il y a un dû, sinon « Total » en `muted`), bouton « Lot » en `trailing` → S07 (lots **ouverts** seulement, **sans** « Sans lot ») ; le choix écrit T13 et la ligne quitte la section sans attendre le rafraîchissement (rollback + toast portant la raison si refus, toast « Rattaché à Commande de mars » au succès). Tap sur la ligne hors du bouton « Lot » → S01.
     - **Amendement J13 — la zone est BORNÉE à 4 lignes**, puis « Afficher les 13 autres » (dépli local de la page déjà chargée), puis « Afficher plus » (page serveur suivante de 100). Livrée telle que spécifiée — 100 lignes d'emblée —, la zone poussait la liste des **lots**, sujet de l'écran, très loin sous le pli : 17 documents sans lot suffisaient. Le compte reste **exact** dans l'en-tête (`· 17`) et la légende dit toujours ce qui n'est pas montré (« 4 affichés sur 17 ») : **jamais tronqué en silence**. Le rangement d'un document reste à **3 taps** — la première rangée est visible sans rien déplier.
     - **Amendement J13 — ordre à l'écran** : le titre de l'écran (`SectionHeader` « Lots » + « Nouveau lot », zone 1) est rendu **avant** la zone 0. La zone 0 reste au-dessus des *listes* de lots, ce qui est le but de l'écart ; la mettre au-dessus du TITRE laissait une liste sans nom en haut de page et retardait l'affichage du titre (05 §5.1).
  1. `SectionHeader` « Lots » + action « Nouveau lot » ; `ListSection` « Ouverts » ; `ListSection collapsible` « Clos » repliée. Ligne : nom ; légende « arrivée prévue 3 oct. · 12 documents » (ou « créé en septembre » sans date prévue — **amendement J13** : le mois est écrit en entier, et daté dès qu'il n'est pas de l'année en cours, « créé en septembre 2025 » ; « créé en sept. » seul ne disait pas de quel septembre il s'agissait), complétée de « · 120 € à encaisser » (`Money warning`) quand le lot porte une créance ; à droite « Marge nette du lot » (`Money`, un seul montant en `trailing`) → E06. **Le nombre de documents compte TOUT ce qui est rattaché, annulés compris** : c'est le même nombre que le refus de suppression de E06, et deux nombres différents pour la même chose sont interdits (05 §5.4). Encaissé, pourcentage, coûts d'achat et dépenses se lisent sur E06 (décision « Simplifier » de 02 §4.4).
- **Action principale** : « Nouveau lot » → E21 puis E06 du lot créé.
- **Actions secondaires** : déplier « Clos » ; déplier « À rattacher ».
- **Gestes** : tap ; pull-to-refresh.
- **États** : *chargement* : squelette de deux sections (en-tête 29 px, rangées 56 px) ; *vide de départ* : **amendement J13** — `EmptyState done` « Aucun lot. « Nouveau lot » en ouvre un. », sans second bouton : l'action de création est déjà dans l'en-tête, et deux chemins visibles vers la même destination sont interdits (05 §5.3) ; *vide de filtre de la zone 0* : « Rien ne correspond à « … » » avec les champs couverts ; *erreur* : `ErrorBanner` (« Lots indisponibles. »).
...
- **Zones.**
  1. En-tête : nom (`InlineNameEditor`) ; légende « Ouvert · arrivée prévue 3 oct. » (tap sur la date → sélecteur natif, effaçable) ; menu « ⋯ » : « Liste pour le fournisseur », « Clôturer » / « Rouvrir », « Supprimer le lot ».
     - **Amendement du 24/09/2026 — « Liste pour le fournisseur »** : sheet des lignes des documents **non annulés** du lot (commandes en attente comprises : c'est ce qu'on fait préparer), **groupées par client** dans l'ordre des demandes (un même nom se rejoint d'un document à l'autre, deux clients de passage restent distincts), un parfum identique fusionné (« ×2 »). Chaque ligne dit parfum · marque · contenance, **jamais un prix**. Cases à cocher : tout est coché sauf ce qui est déjà remis au client ; « Tout cocher / Tout décocher ». « Envoyer au fournisseur… » (feuille de partage iOS) et « Copier la liste ». Modèle pur : `src/features/batches/components/supplier-model.ts`.
  2. **Tuiles fixes** (grille stable, toujours les cinq) : « Encaissé » (`display`, pleine largeur) et « Marge nette » + % ; « À encaisser », « Coûts d'achat » (légende « 2 au coût à compléter, comptés 0 € » quand il en reste), « Dépenses ». Chacune porte `data-kpi-tile="<libellé>"` : une assertion sur la grille entière passe sur la mauvaise tuile (relevé à J13).
     - **Amendement J13** : « À encaisser » ne prend le ton `warning` que s'il est **non nul** — `warning` est le ton d'un montant non reçu, et à zéro il n'y a rien à signaler (05 §3.3). La tuile reste affichée à zéro : la grille des cinq ne bouge pas d'un lot à l'autre (exception assumée à « une tuile à zéro disparaît », 05 §5.3).
     - **Amendement du 24/09/2026** : la tuile « Achat des parfums » (libellé d'écran de « Coûts d'achat ») s'ouvre sur place (`KpiTile` `onClick`) : **l'achat relu en dinars**, avant conversion — total payé au fournisseur, un groupe par taux pratiqué (« 92 700 DA au taux 277 = 334,65 € »), puis ligne par ligne (« 2 × 9 000 DA · taux 277 · Élise Martin »). Même périmètre que la tuile (documents engagés) : la somme en euros est la sienne au centime (test de parité `tests/db/batches-queries.test.ts`) ; les lignes sans achat renseigné sont comptées à part.
     - **Reste à faire (J12)** : le tap « Marge nette » → **S19** au périmètre du lot. S19 arrive avec la compta ; la tuile est pour l'instant en lecture seule, sans `href` — un raccourci ne mène jamais à un écran absent.
...
- **Zones.**
  ...
  4. **Client** : un champ « Nom du client », rien d'autre (amendé le 24/09/2026 : plus de rangée « Client de passage », plus de contact). Sous le champ, jusqu'à 4 fiches qui ressemblent à la saisie ; en toucher une lie le document à cette fiche. Un nom tapé sans choisir est envoyé tel quel (`customer.kind = "named"`, T1) : la fiche au même nom — casse, accents et ponctuation ignorés (`cleNom`), la plus ancienne s'il y a des homonymes — est reprise, sinon une fiche nom seul est créée dans la transaction. L'aide le dit : « Client enregistré : la vente ira sur sa fiche. » ou « Nouveau client : sa fiche sera créée à l'enregistrement. ». Le nom est **obligatoire partout** (amendé le 24/09/2026), vente payée comprise : chaque vente se range sur une fiche. Sans nom, le CTA est « Choisir le client » et y place le focus ; son résumé dit le dû s'il en reste (« Nécessaire pour suivre les 70 € à encaisser »), sinon « Chaque vente se range sur une fiche client ». T1 refuse de même une création sans nom.
  5. **Articles.**
     - Le bouton-champ « Rechercher un parfum » → S05 est **en tête** de la zone, au-dessus des récents (amendé le 24/09/2026).
     - *Sans ligne* : grille « Vendus récemment » 2 colonnes × 4 (tuile ≥ 88 px de haut : vignette, nom, marque, « 80 ml · 120 € » = dernier volume et dernier prix pratiqués ; badge « Rupture » ou « Masqué » si c'est le cas). Retaper une tuile ajoute 1 à la quantité (« ×2 » sur la tuile).
     - *Avec lignes* : une `Card` par ligne — vignette, nom · marque ; chips de volume « 10 · 50 · 80 » (contenances réelles ; une ligne ajoutée sans volume mémorisé prend 80 ml, `DEFAULT_VOLUME_ML` ; changer de volume re-remplit prix, coût et taux depuis `PerfumePricing`) ; `Stepper` de quantité (à 1, le « − » devient « Retirer », toast « Ligne retirée » + « Annuler ») ; prix `MoneyInput` (aide « dernier prix : 110 € » si le prix saisi s'en écarte, « Aucun prix mémorisé pour 50 ml » si vide) ; `GiftToggle` « Offert » (décoché : dernier prix restauré) ; rangée repliée « Coût 9 000 DA · taux 277 · 32,49 € » (tap : champs coût DZD, taux et « Note » de la ligne — note par ligne de l'existant, 01 §3.1, `SaleLine.note` ; une note saisie s'affiche en légende de la rangée) ou « Coût à compléter » (`warning`, non bloquant) ; un badge au plus : « Rupture » / « Stock bas » / « Masqué ». Sous les lignes : « Rechercher un parfum », puis le bandeau horizontal des récents.
...
- **Zones.**
  ...
  2. Zone épinglée : `SearchField` (insensible aux accents) ; chips à compteur rendus seulement s'ils discriminent — Parfums : « Visibles (n) · Masqués (n) · Stock bas (n) · Rupture (n) » ; Marques : « Visibles (n) · Masquées (n) · Gammes complètes (n) » (« Visibles » ajouté le 24/09/2026) ; filtre venu d'un lien toujours affiché et effaçable.
...
- **Gestes** : tap (l'œil est optimiste, rollback + toast portant la raison du refus : « Rends d'abord la marque Dior visible. », « Ajoute un visuel pour publier ce parfum. » — textes uniques de `src/domain/publication.ts`, les mêmes que le serveur, 04 §12) ; pull-to-refresh. Rendre visible une marque en mode Sélection qui a des parfums masqués avec visuel (`republishable` rendu par `setBrandVisibilityAction`) → `ConfirmDialog` « Dior est visible. Republier aussi ses 8 parfums qui ont un visuel ? » (« Republier » → `republishBrandPerfumesAction` / « Plus tard »).
...
- **Zones.**
  ...
  6. **Activité** : « Vendu 12 fois · dernier le 14 sept. » — « 12 » compte les **unités** vendues (somme des quantités des lignes du parfum, décision du 17/09/2026 : c'est ce qui sert au réassort), la date est celle du document le plus récent (`orderedAt`), documents annulés exclus ; absent si jamais vendu.
...
- **Actions secondaires** : « Modifier » (en-tête) → E19 ; menu « ⋯ » : « Dupliquer » → `/admin/catalogue/parfums/nouveau?dupliquer=<id>` (marque et tarifs repris, nom et visuel vides — visuels story non repris ; aucune action : E19 lit `perfumeDuplicationDraft(id)` et pré-remplit, rien n'est écrit avant « Ajouter au catalogue », comme A-9) ; « Ajouter des visuels », « Partager / Enregistrer », « Retirer » (zone 7).
...
- **Données** : `Perfume` (`status`, `isFeatured`, `stock`, `image`, `imageLight`), `Brand`, `PerfumePricing` ×3, activité des `SaleLine` du parfum (unités, documents, dernier `orderedAt`, hors annulés ; fiche lue sans cache, 04 §10.4), `PerfumeMedia` du parfum (`url`, `label`, `width`, `height`, par `sortOrder`) ; écritures `setPerfumeStatusAction`, `setPerfumeFeaturedAction` (04 §3.4), puis `revalidateAdminCatalogue()` ; visuels : `createImageUploadUrlAction({ usage: "story", perfumeId })`, `addPerfumeMediaAction`, `removePerfumeMediaAction` (04 §12 — chemin décidé par le serveur ; n'invalident pas la vitrine).
...
- **Zones.**
  ...
  2. **Identité** : rangée « Marque » → S05 (mode marques) ; « Nom » (aide « Sera enregistré : Sauvage » quand la normalisation change la saisie). La marque choisie (existante) ou saisie (« Créer la marque « … » ») part **avec** la fiche et n'est créée qu'à « Ajouter au catalogue » ou « Enregistrer », dans la même transaction (04 §3.4) : abandonner le formulaire ne laisse aucune marque orpheline ; si une marque équivalente existe entre-temps, la fiche y est rattachée avec la notice « Rattaché à Louis Vuitton, déjà au catalogue. ».
  3. **Tarifs** : une sous-section par contenance réelle — « 80 ml » ouverte, « 10 ml » et « 50 ml » repliées si vides — prix €, coût DZD, taux (placeholder = taux par défaut de `Setting`), légende « Coût en euros 32,49 € » ; « Retirer ce volume ». L'enregistrement envoie la grille **complète** : un volume retiré est supprimé, un coût ou un taux vidé est effacé ; un volume gardé sans prix (ou à 0 €) est refusé sous le champ : « Indique le prix du 80 ml, ou retire ce volume. » ; un coût sans taux est accepté (le taux par défaut sera proposé à la vente).
...
- **Action principale** : « Enregistrer » (modification : fiche + tarifs en **une** écriture, `updatePerfumeAction` avec la grille, 04 §3.4, A-6) ; « Ajouter au catalogue » (création), qui guide tant qu'il manque la marque ou le nom (« Choisir la marque », « Saisir le nom »). Succès → E16 du parfum, pulse.
...
- **États** : *chargement* : squelette ; *introuvable* : `EmptyState` + « Retour au catalogue » ; *envoi d'image échoué* : `ErrorBanner` dans le champ « Envoi impossible — Réessayer » ; *enregistrement refusé* : message sous le champ (`VALIDATION`) ou toast (`CONFLICT`, ex. « Dior a déjà un parfum nommé Sauvage. », doublon au sens de `cleNom` : même nom sous une autre graphie), saisie conservée ; *création sous règle de visibilité* : le parfum est enregistré masqué, toast d'information portant la raison du domaine — « Sauvage ajouté, masqué : ajoute un visuel pour publier ce parfum. » (ou « … : rends d'abord la marque Dior visible. ») ; *modification qui fait perdre la visibilité* : même forme, « Sauvage masqué : » suivi de la raison (visuel retiré : « Sauvage masqué : ajoute un visuel pour publier ce parfum. » ; marque changée pour une marque masquée : « … : rends d'abord la marque Dior visible. »), la mise en avant est perdue avec la visibilité (la fiche ne rend jamais un parfum visible : c'est l'interrupteur de E16).
...
- **Données** : `Perfume`, `Brand`, `PerfumePricing`, `Setting.defaultExchangeRate` ; écritures du module catalogue (`createPerfumeAction` et `updatePerfumeAction`, grille tarifaire comprise ; `deletePerfumeAction`, `createImageUploadUrlAction`), `revalidateAdminCatalogue()` automatique (04 §10.2) ; `?dupliquer=<id>` : lecture `perfumeDuplicationDraft(id)`. Images envoyées puis abandonnées : `scripts/storage-orphans.ts` (04 §7.3).
...
- **Zones.**
  ...
  2. **Nom** (normalisé ; si une marque équivalente existe : notice `info` « Louis Vuitton existe déjà — Ouvrir »). En création, si le serveur trouve une équivalente (saisie croisée), il la rend sans rien créer, avec la notice « Louis Vuitton existe déjà au catalogue : elle a été sélectionnée. » ; en modification, renommer vers le nom d'une autre marque est refusé (`CONFLICT`) : « La marque Louis Vuitton porte déjà ce nom : ouvre-la plutôt que d'en renommer une autre. »
  3. **Mode** : cartes radio « Sélection — seuls les parfums publiés apparaissent » / « Gamme complète — une carte unique avec le logo ».
  4. **Vitrine** : interrupteur « Visible » (verrouillé en gamme complète sans logo : « Ajoute un logo pour publier une gamme complète. » — le serveur refuse de même, en `CONFLICT`, tout enregistrement d'une gamme complète visible sans logo : jamais masquée en silence) ; rangée « Lien public » (lecture seule, bouton « Copier ») : l'URL de la vitrine filtrée sur la marque (`?maison=<slug>`), **stable au renommage**.
...
- **Dialogues** : un enregistrement qui masque la marque ou la passe en « Gamme complète » alors qu'elle a des parfums visibles → `ConfirmDialog` de la réserve serveur (T14, S18) « Masquer Dior ? » / « Passer Dior en gamme complète ? » : « Ses 14 parfums seront masqués sur la vitrine. » (le nombre compte les parfums visibles ; un seul : « Son parfum visible sera masqué sur la vitrine. ») ; un enregistrement qui la rend visible en « Sélection » alors que des parfums avec visuel sont masqués (`republishable` > 0) → « Republier les 8 parfums qui ont un visuel ? » (`republishBrandPerfumesAction`) ; supprimer → « Supprimer Dior et ses 14 parfums ? Ils disparaissent de la vitrine. Les ventes passées gardent leurs noms. » + « Masquer plutôt ».
...
- **Données** : `Brand` (`name`, `slug` en lecture seule, `catalogMode`, `status`, `image`, `imageLight`), comptes de `Perfume` (total, visibles, masqués avec visuel) ; `createBrandAction`, `updateBrandAction` et `setBrandVisibilityAction` (T14 ; rendent `{ brand, hiddenPerfumes, republishable }`), `republishBrandPerfumesAction`, `deleteBrandAction` ; `revalidateAdminCatalogue()`.
...
- **Variantes.**
  ...
  | Livrer | E10 (glissement « Livrer »), S01 segment « Livrée » avec dû > 0 | « Livrer · Fares » | dû / dû | « Encaisser 60 € et livrer » ; secondaire « Livrer sans encaisser » | T7 (un solde, sans confirmation automatique préalable) + T4 en une action transactionnelle, `deliverAndCollectAction` (04 §3.4) ; réserves de livraison calculées avec le payé après encaissement |
...
- **Après succès** : la sheet se ferme ; toast « 80 € encaissés · Espèces » + « Annuler » (5 s) ; pulse sur l'élément d'origine. « Annuler » passe **toujours** par T4b (`revertDocumentChangeAction` avec le jeton rendu par l'écriture, jamais `voidPaymentAction`) : contre-passation des paiements du geste et retour à l'état d'avant, en une transaction (03 §4.3). **Solde**, **Acompte** sur une commande déjà confirmée : contre-passation seule (le statut n'avait pas changé). **Acompte** qui a confirmé une commande « En attente » : contre-passation **et** retour « En attente » — si un autre paiement net subsiste, la commande reste confirmée et le toast de l'annulation le dit : « La commande reste confirmée : 120,00 € à encaisser. ». **Tout encaisser** : tous les paiements du geste contre-passés ensemble, chaque document revenant à son état d'avant selon la même règle. **Livrer** : contre-passation, retour à l'état d'avant la livraison (statut, pointage par ligne, stock). Refus si un document a changé depuis (« Ce document a changé depuis ce geste : rien n'a été annulé. Corrige-le depuis sa fiche. ») ou si le jeton a plus de 10 minutes (« Trop tard pour annuler ce geste : corrige-le depuis la fiche du document. »).
...
- **États** : comme S02, plus les refus du serveur : *remboursement à l'annulation au-delà du payé* : « Le remboursement dépasse ce qui a été payé (60,00 €). » ; *remboursement au-delà du plafond* (payé net d'un document annulé, trop-perçu sinon) : « Le montant dépasse ce qui peut être remboursé (10,00 €). » ; *document déjà annulé entre-temps* : succès si c'est le renvoi de la même annulation, sinon « Cette commande est déjà annulée : rembourse depuis sa fiche. » (« Cette vente… ») ; *document livré* : la réserve de transition « Cette commande était livrée : le stock est restitué. » est confirmée avant l'annulation (S18).
...
- **Règles du serveur** : le paiement corrigé garde la **nature** de l'original (un acompte reste un acompte) ; poche ou date non envoyées = inchangées ; si seuls le moyen ou la note changent, la pièce est modifiée en place, sans contre-passation ni mouvement ; si rien ne change, succès sans écriture ; le plafond du nouveau montant (reste dû, ou remboursable pour un remboursement) est lu **après** la contre-passation de l'ancien ; un remboursement se corrige de la même façon ; un paiement déjà annulé, ou qui est lui-même une annulation, ne se corrige pas (`CONFLICT`).
- **États** : comme S02.
- **Composants** : `Sheet`, `MoneyInput`, `Chip`, `FormField`, `Button`, `ErrorBanner`.
- **Données** : `Payment`, `CashMovement`, `Pocket` ; écriture T8 (annuler + nouveau, une transaction ; moyen ou note seuls : mise à jour de la pièce).
...
- **Zones — marques** (depuis E19) : marques récentes, recherche ; « Créer la marque « … » » seulement si aucune marque équivalente (`cleNom`) — ce tap n'écrit rien : il pose la marque saisie sur la fiche (`{ kind: "new", name }`), créée à l'enregistrement de E19 dans la même transaction ; si le serveur trouve alors une équivalente, la fiche y est rattachée et la notice `info` le dit : « Rattaché à Louis Vuitton, déjà au catalogue. ».
...
- **But.** Lier une fiche en 1 tap depuis S01 « Lier une fiche », ou en créer une sans quitter la saisie. (Le composeur E11 ne l'ouvre plus : son client est un champ nom, amendé le 24/09/2026.)
- **Zones.**
  1. (Supprimée le 24/09/2026 : rangée « Client de passage », qui ne servait qu'au composeur.)
...
- **Action principale** : « Enregistrer (3 changements) » (seul le différentiel part, T13). Le toast dit **ce qui a été appliqué**, pas ce qui a été demandé : « 3 documents rattachés », « 1 retiré du lot », « 2 documents rattachés · 1 retiré du lot » ; jamais « mis à jour » quand rien ne l'a été (« Aucun changement : tout était déjà à jour. »).
  - **Amendement J13 — pas de « ignoré »** : T13 est **tout-ou-rien** (04 §4.3, §T13). Un document annulé ou déplacé entre-temps fait refuser l'enregistrement **entier**, avec sa raison (`CONFLICT`) ; il n'y a donc jamais de « 2 rattachés · 1 ignoré : annulé depuis » à annoncer. Ce qui peut différer du demandé, c'est le nombre de documents réellement **écrits** (ceux déjà dans l'état visé ne comptent pas) : « 2 documents enregistrés · 1 déjà à jour ». 04 fait foi sur les transactions (07 §3.0.2).
  - **Amendement J13 — la recherche filtre sur l'appareil**, sur les candidats déjà servis par le RSC (plafond de 300, le compte réel étant toujours dit). Une recherche qui repasserait par l'URL relancerait la requête et rouvrirait exactement la boucle que l'écart du 17/09/2026 ferme.
  - **Amendement J13 — le pied s'efface pendant la frappe** : clavier ouvert sur un iPhone SE, titre + recherche + pied ne laissaient que quelques dizaines de pixels à la liste (invariant « sheet écrasée »). Même remède que la fiche document. Le CTA revient dès que le clavier se ferme.
...
- **Zones.** Nom (`InlineNameEditor`, sauf « Non attribué ») ; type ; solde (`display`) ; légende « Poche proposée par défaut » ou bouton « Définir par défaut » ; 10 derniers mouvements + « Voir tout » → E04 `?poche=` ; « Archiver » (seulement à solde nul et hors poche système ; sinon désactivé : « Solde non nul : transfère d'abord 120 € » ; le serveur refuse de même : « Solde non nul : la poche « Banque » contient 120,00 €. Transfère d'abord son solde, puis archive-la. » ; archiver la poche par défaut remet la poche proposée à « Non attribué », `Setting.defaultPocketId` à `NULL`) ; « Supprimer » si la poche n'a jamais eu de mouvement (`deletePocketAction`, 04 §3.4 ; `ConfirmDialog` « Supprimer la poche Banque ? » qui, si son solde d'ouverture n'est pas nul, dit « Ses 200 € d'ouverture sortent de la Trésorerie. »).
...
- **Zones.** `SegmentedControl` « Transfert · Ajustement · Paiement fournisseur » (masqué en mode Répartir, titre « Répartir le non attribué »).
  ...
  - *Transfert* : « De » et « Vers » (chips) ; montant + chip « Tout » ; date ; note. « Non attribué » ne peut jamais passer sous zéro ; une autre poche qui passerait en négatif → réserve `ConfirmDialog` du serveur « Mettre « Banque » en négatif ? » (texte et bouton : S18). CTA « Transférer 300 € vers Banque » (T11).
...
- **Données** : `Pocket`, soldes, `CashMovement` (`TRANSFER`, `ADJUSTMENT`, `SUPPLIER`) ; écritures T11 et écritures simples du module trésorerie. Chaque mode envoie un `id` facultatif généré à l'ouverture de la sheet : pour un transfert, c'est l'identifiant de la jambe sortante **et** du groupe de transfert ; pour un ajustement ou un paiement fournisseur, celui du mouvement — un renvoi après coupure rend l'écriture déjà faite, sans doublon (04 §3.6).
...
- **Données** : `Pocket`, `Setting.defaultPocketId` ; `createPocketAction` (`id` facultatif pour un renvoi sans doublon ; la poche est ajoutée **en fin** d'ordre, rang max + 1 ; « Proposer par défaut » = `makeDefault`, qui mémorise la poche dans la même transaction).
...
| Transfert qui met une poche en négatif (S15) — réserve serveur de T11 | « Mettre « Banque » en négatif ? » | « « Banque » contient 280,00 € : son solde passera à −20,00 €. » | « Transférer » (`primary`) |
...
| Masquer une marque / passer en gamme complète (E15 œil, E17) — réserve serveur de T14, seulement si la marque a des parfums visibles | « Masquer Dior ? » / « Passer Dior en gamme complète ? » | « Ses 14 parfums seront masqués sur la vitrine. » (nombre de parfums **visibles** ; un seul : « Son parfum visible sera masqué sur la vitrine. ») | « Confirmer » (`primary`) |
...
| Chips de date | E11, S02, S12, S15 | « Aujourd'hui », « Demain », « Hier », « Choisir… » | Date de valeur (paiement, dépense, mouvement) jamais dans le futur : un **jour** de Paris postérieur à aujourd'hui est refusé par le serveur en `VALIDATION` sous le champ — « Choisis une date passée : un paiement ne se date pas dans le futur. » (« une dépense », « un mouvement ») ; une heure postérieure **le jour même** est ramenée à « maintenant » | Sélecteur natif |
| Touche Entrée | `MoneyInput` d'une sheet d'argent | Déclenche le CTA | Seulement si le montant est valide | CTA |
| Toast « Annuler » (5 s) | Après encaisser, livrer, changer de statut, supprimer un document sans paiement, supprimer une fiche client ou un parfum | Défait l'écriture (T4b pour tout encaissement, livrer et changer de statut — contre-passation des paiements du geste et état d'avant, pointage et stock compris ; suppression différée pour le reste) | Un seul toast à la fois ; un nouveau geste valide le précédent | Menus de S01 (« Annuler ce paiement »), statut |
...
|---|---|
| Écriture d'argent (encaisser, livrer et encaisser, tout encaisser) | Toast « Annuler » 5 s = T4b : contre-passation des paiements du geste, et retour à l'état d'avant quand le geste avait changé le statut (« Livrer et encaisser », acompte qui a confirmé) ; ensuite « Annuler ce paiement » dans S01, qui ne change plus le statut |
...
7. **Paramètres d'URL** retenus : `filtre`, `vue`, `periode`, `ref`, `anciennete`, `stock=bas|rupture`, `visibilite=visibles|masques`, `mode`, `client`, `parfum`, `depuis`, `dupliquer`, `nom`, `mois`, `poche`, `jour` ; redirections complémentaires du §1.6.
