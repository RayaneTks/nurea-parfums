...
| `scripts/repetition/preprod.ts` (`npm run repetition:preprod`) | Remplit une préproduction **distante** avec les données réelles migrées (E-5). Même chaîne que `repetition:refresh`, mais la cible est une base gérée (Neon, Supabase) qu'on ne peut ni supprimer ni recréer : on vide `public` et `legacy` **sans les supprimer** (`lib/vidage.ts`, le balayage du retour arrière), puis ancien schéma, données de l'instantané, `reference → expand → reprise --apply → contract → verify`. Source lue dans `SOURCE_DATABASE_URL` (jamais `.env`), en lecture seule stricte. **Refuse la production comme cible, sans exception**, même avec `--confirm-host`. | `--confirm-host` (obligatoire) ; `--instantane` ; `--out` | Oui, sur la préproduction uniquement |
