...
| `src/design/globals.admin.css` (`.admin-theme` sans fond, `.admin-paint`, bandes d'empilement), `src/design/tokens.ts`, `src/ui/primitives/Toast.tsx` (portail), `src/ui/patterns/ConfirmDialog.tsx` (erreur dans la boîte, corps défilant) | Corrections à reporter dans le design system de la branche (05 §2, §2.7, §3.1, §3.2 — « à appliquer ») | J4 (reprise de l'écart) |
...
- **Reprise de l'écart du 17/09/2026** (05, mentions « à appliquer ») : `.admin-theme` ne peint plus de fond (`.admin-paint` sur les conteneurs pleine page) ; bandes d'empilement sheet imbriquée 80/81, modale 90/91, palette 92, toast 100 (`tokens.ts` puis feuille) ; `Toast` portalisé vers `<body>` avec `pointer-events: auto` ; `ConfirmDialog` affiche l'échec dans la boîte et fait défiler son corps. Le design system J4a de la branche a été construit avant ces corrections de production (`3291428`, `12e2327`, `3707715`) et les porte encore.
...
- **Visuels story (écart du 17/09/2026, capacité de production)** : `src/server/catalogue/media.ts` (writer : ajout avec rang calculé et plafond de 24, réordonnancement, retrait qui rend le chemin), actions `addPerfumeMediaAction` (chemin exactement `stories/<perfumeId>/…`, sans `..`, URL recalculée), `reorderPerfumeMediaAction`, `removePerfumeMediaAction` (objet supprimé **après** le commit) ; `createImageUploadUrlAction({ usage: "story", perfumeId })` ; `deletePerfumeAction` qui lit les chemins avant le DELETE ; tags `gestion` + `admin-catalogue` sans invalidation de la vitrine (04 §10.1) ; `src/ui/patterns/MediaGallery.tsx` porté depuis `main` (05 §3.2 : couleurs et z-index en jetons) ; E16 zone 7, légende « · n visuels story » de E15 ; `scripts/storage-orphans.ts` compte `PerfumeMedia.path` ; `tests/db/catalogue-media.test.ts` (04 §16.3).
