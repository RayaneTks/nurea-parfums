...
| B14 | **Fusion** : PR du tag `bascule-<date>` vers `main` (commit de fusion) ; tag `avant-refonte` sur le dernier commit de `main` avant fusion. Le déploiement automatique de `main` reconstruit le même code (aucune migration en attente : la garde laisse passer). | Le nouveau déploiement de production passe B11 à nouveau | Promouvoir de nouveau R |
...
| `scripts/migration/verify-post.ts` (`npm run migration:verify`) | Après le contract : V8 (contraintes validées ou listées), C1–C5 recalculés sur le schéma final contre la référence, invariants de 03 §5.7, comptages vitrine en base. | `--reference <fichier>` ; `--confirm-host` | Non |
...
**Noms fixés ici pour `legacy."MigrationMap"`** (03 §7.3 ne les nomme pas) : `"oldTable" text`, `"oldId" text`, `"newTable" text`, `"newId" text`, `note text`. Une ligne par ancienne ligne reprise ou fusionnée (une `Sale` fusionnée dans le document de sa commande donne `("Sale", <Sale.id>, "SaleDocument", <Order.id>, "paire")`).
...
- une poche **archivée à solde non nul** fait échouer V6 : la première répétition (J2) dit si le cas existe ; s'il existe, la décision (désarchiver et lister, ou transférer) est prise avec le gérant et **codée dans la reprise** avant J3.
...
**Comptages** (bloquants, = V5) : documents = commandes + ventes − paires ; lignes = `OrderItem` hors paires + `SaleItem` ; chaque `PaymentTransaction` a son `Payment` ; chaque ancien `CashMovement` est classé dans exactement une catégorie de 03 §7.6. **Vitrine** (bloquant, = V9) : parfums publiés, cartes « gamme complète », marques Explorer, identiques avant et après.
...
| `src/lib/admin/resoudMarque.ts` | `src/server/catalogue/resoudMarque.ts`, écritures par le writer catalogue | J11 |
...
| Anciennes formules d'Encaissé et d'À encaisser (`src/server/kpi/queries.ts`, `src/server/orders/financials.ts`, `src/server/collect/queries.ts`) | Copiées **en SQL** dans `scripts/migration/reference.ts` uniquement | J2 |
...
**Contenu.**
- `scripts/migration/reference.ts`, `apply-sql-migration.ts`, `reprise.ts` (+ `reprise/3a…3i`), `verify-post.ts` (§2.2) ; `scripts/repetition/refresh.ts` complété (chaîne complète, chronométrage, comparaison au rapport précédent).
...
- [ ] `npm run repetition:refresh` : sortie 0 ; V8 listé ; C1–C5 verts ; résidu C3 = 0,00 € ; durées par étape consignées dans la PR.
...
- `src/server/payments/writer.ts` et `actions.ts` : `recordPaymentAction` (T7, nature fixée par le serveur, plafond au dû, confirmation automatique sans réserve seulement), `voidPaymentAction`, `correctPaymentAction`, `refundAction` (T8), **`collectAllAction`** (A-5).
- Documents : T1 **avec paiements** (Σ ≤ total, poche mémorisée), T5 `cancelDocumentAction` (remboursements proposés, plafond au payé, verrou exclusif des poches de sortie), **`deliverAndCollectAction`** (A-5), **`revertDocumentChangeAction`** (T4b de 03 §4.3 : filet « Annuler » du toast après livrer, « Livrer et encaisser », changer de statut ou encaisser un acompte qui a confirmé ; `changeDocumentStatusAction`, `recordPaymentAction` et `deliverAndCollectAction` renvoient le jeton d'annulation) ; garde des lignes reprises hors règles (03 §4.3) dans le writer `documents`.
...
**Contenu.**
- Routes de lecture `GET /api/admin/search` et `GET /api/admin/picker?v=` (`defineReadRoute`, 04 §3.5), `src/server/search/queries.ts`, version et contenu du sélecteur dans `src/server/catalogue/queries.ts`.
...
- **E10** Commandes : vues « À livrer · Livrées · Annulées », sections par urgence, chips, recherche, « Afficher plus », `SwipeableRow` (« Livrer » à droite, « Encaisser » à gauche, 06 §4.2).
...
**Contenu.**
- `src/contracts/catalogue.ts` ; `src/server/catalogue/writer.ts` complet, `actions.ts` (`createPerfumeAction` et `updatePerfumeAction` **avec la grille tarifaire**, A-6 ; `deletePerfumeAction` ; `setPerfumeStatusAction` ; `setPerfumeFeaturedAction` ; `setPerfumeStockAction` ; `createBrandAction`, `updateBrandAction`, `deleteBrandAction`, `setBrandVisibilityAction` T14 ; `createImageUploadUrlAction`), `queries.ts` (instantané admin sous le tag `admin-catalogue`, fiche), `resoudMarque.ts` (repris), `storage.ts` (URL signée ; suppression après commit avec la garde de préfixe de §1.3).
- `src/features/catalogue/components/image-convert.ts` : recadrage portrait 1024 × 1536 pour un parfum, **aucun recadrage** pour un logo.
...
- [ ] `npm run test:layout` : E05 (ouverts et clos), E06, E21 clavier ouvert, S12 clavier ouvert, S13 par `?assigner=1`.
...
Chaque capacité de 01 §3 est **présente** (vérifiée à l'endroit indiqué) ou **abandonnée** par une décision de 02 §4 (citée). « e2e » renvoie aux fichiers de `e2e/parcours/`, « db » à `tests/db/`. La colonne « Vu » est cochée pendant la recette de J16, en préproduction, avec le gérant.
...
| NR-1.3 | Création : client lié ou libre, catalogue ou hors catalogue, volumes 30/50/100, don, coût DZD + taux, **note par ligne**, livraison, notes | E11 mode Commande, S05, S06 ; note par ligne (A-12) | e2e `commande-saisie` ; db `t01` | J9 | [ ] |
...
| NR-5.11 | Grille tarifaire par (parfum, volume) | E19 zone 3, un seul enregistrement (A-6) | e2e | J11 | [ ] |
