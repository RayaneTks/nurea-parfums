...
enum PaymentKind {
  ...
  DEPOSIT
  /// Solde : encaissé sur un document livré (dont « Reçu maintenant » d'une vente directe), et paiement de
  /// « Livrer et encaisser », reçu à la livraison dans le même geste (sans confirmation automatique préalable).
  BALANCE
...
| `Setting` (taux, poche par défaut) | module **réglages** (appelé par les encaissements, les dépenses et les poches pour la poche proposée) | écran Réglages ; mémoire de poche (N2) : choisir « Non attribué » se mémorise `NULL`, un T1 à plusieurs paiements mémorise la poche du premier, archiver la poche par défaut la remet à `NULL`. Sans ligne `Setting` (base neuve), la lecture rend le taux 277 et la poche `NULL`, la première écriture fait un `upsert` de l'id 1 (04 §7.4) |
...
|---|---|---|---|
| T1 | Créer un document, avec acompte ou « Reçu maintenant » éventuels | `SaleDocument` (statut + horodatages), `SaleLine` ×n, (`CashMovement` + `Payment`) ×n, `Perfume.stock` (si né `DELIVERED`), `PerfumePricing` (lignes non offertes), `Setting.defaultPocketId` | `Perfume` ; Σ paiements ≤ total, sinon `VALIDATION` « Le montant reçu dépasse le total (120,00 €). » |
...
| T4b | Défaire un changement d'état (filet « Annuler » du toast, 5 s) : livrer, changer de statut (réactiver compris), encaisser (acompte ou solde), « Livrer et encaisser », « Tout encaisser » | `SaleDocument` (statut et horodatages d'avant), `SaleLine.deliveredQuantity` (valeurs d'avant), `Perfume.stock` (delta), et pour un geste qui a encaissé : contre-passation de ses paiements (T8 « annuler », même transaction) | `SaleDocument FOR UPDATE` ; **sans réserve de transition**. Le geste (tout T4, réactivation comprise ; tout T7 ; actions composées « Livrer et encaisser » et « Tout encaisser ») renvoie un jeton d'annulation lu sous verrou après ses écritures, signé HMAC-SHA256 (`ADMIN_JWT_SECRET`) : type de geste, `issuedAt`, et par document l'état d'avant (statut, horodatages, livré par ligne), l'empreinte de l'état d'après (statut, horodatages, `updatedAt`, lignes, identifiants des paiements) et les paiements créés. « Tout encaisser » rend un jeton pour tous ses documents, annulés ensemble en une transaction ; un geste renvoyé (même identifiant, rien d'écrit) rend `undo: null`. Refus `CONFLICT` si l'état courant diffère de l'empreinte (« Ce document a changé depuis ce geste : rien n'a été annulé. Corrige-le depuis sa fiche. ») ou après 10 minutes (« Trop tard pour annuler ce geste : corrige-le depuis la fiche du document. »). Pour un encaissement ou « Tout encaisser » seulement : le retour en `PENDING` d'une commande confirmée par ce paiement exige un payé net nul après contre-passation ; sinon le statut reste et la notice le dit (« La commande reste confirmée : 120,00 € à encaisser. »). Stock : delta borné à 0 comme une réserve confirmée (§4.7) |
...
| T8 | Annuler, corriger ou rembourser un paiement | Annuler une entrée (DEPOSIT, BALANCE) : `CashMovement` (contre-passation, négative) + `Payment` REFUND. Annuler un remboursement (REFUND) : `CashMovement` (contre-passation, positive) + `Payment` d'entrée, DEPOSIT si le document n'est pas livré, BALANCE sinon (§4.4). Corriger : annuler + nouveau (`CashMovement` + `Payment`). Rembourser : `CashMovement` négatif daté du jour + `Payment` REFUND. Corriger garde la **nature** de l'original ; poche ou date absentes = inchangées ; seuls le moyen ou la note changent ⇒ `Payment` modifié en place (colonnes descriptives), sans mouvement ; rien ne change ⇒ succès sans écriture ; un remboursement se corrige aussi. **Annuler ne change jamais le statut** (seul T4b, dans la fenêtre du toast, rétablit l'état d'avant) | `SaleDocument FOR UPDATE` ; unicité `reversesId` : un paiement déjà contre-passé ⇒ `CONFLICT` « Ce mouvement a déjà été annulé. » ; `Pocket FOR UPDATE` des poches de sortie : « Non attribué » jamais négatif ; pas de plafond au dû pour l'annulation d'un remboursement (elle rétablit l'état d'avant celui-ci) ; corriger : plafond du nouveau paiement lu **après** la contre-passation (reste dû pour une entrée, remboursable pour un remboursement) ; rembourser : plafond = payé net d'un document annulé, trop-perçu sinon, « Le montant dépasse ce qui peut être remboursé (xx,xx €). » |
| T9 | Ajouter une dépense de lot | `CashMovement` (EXPENSE, négatif), `BatchExpense`, `Setting.defaultPocketId` | poche non archivée, `Pocket FOR UPDATE` (toute sortie, 04 §4.2) |
| T10 | Supprimer une dépense (contre-passation) | `CashMovement` (EXPENSE, positif, `reversesId`) | unicité `reversesId` ; dépense déjà supprimée ⇒ succès sans écriture (`deleted: false`, renvoi après coupure) |
...
| T12 | Annuler un mouvement manuel (dont un transfert : les deux jambes) | `CashMovement` ×1 ou ×2 | unicité `reversesId` : mouvement déjà contre-passé ⇒ `CONFLICT` « Ce mouvement a déjà été annulé. » |
...
| T15 | Archiver une poche | `Pocket.archived`, `Setting.defaultPocketId := NULL` si c'était la poche par défaut | `Pocket FOR UPDATE` ; poche non système ; solde nul exigé, sinon `CONFLICT` « Solde non nul : la poche « Banque » contient 120,00 €. Transfère d'abord son solde, puis archive-la. » |
...
| Paiement saisi par erreur | Annuler ce paiement | REFUND dont le mouvement **contre-passe** l'original (même poche, **même date de valeur**, montant opposé) : la période concernée redevient juste, les deux lignes restent au journal (repliées). **Remboursement saisi par erreur** : la contre-passation est positive, elle est donc portée par un paiement d'entrée — DEPOSIT si le document n'est pas livré, BALANCE sinon — et non par un REFUND (le trigger de cohérence exige REFUND ⇔ montant négatif, §4.10). Le statut du document ne change pas : sur un document engagé, la confirmation dit ce qui restera à encaisser (06 S18). **Une contre-passation ne se contre-passe pas** : annuler le paiement qui porte une annulation ⇒ `CONFLICT` « Ce paiement annule déjà un autre paiement : il ne s'annule pas lui-même. » |
| Paiement erroné (montant, date, poche, moyen, note) | Corriger | Annuler + nouveau paiement de même nature, une transaction (T8). Moyen ou note seuls : modifiés en place sur la pièce, sans mouvement. Une erreur de **poche seule** se corrige aussi par un transfert. |
...
| Mouvement manuel (transfert, ajustement, paiement fournisseur) | Annuler | Contre-passation (les deux jambes d'un transfert, nouveau `transferGroupId`). Une contre-passation ne se contre-passe pas : « Ce mouvement est lui-même une annulation : il ne s'annule pas. » ; un mouvement déjà annulé : « Ce mouvement a déjà été annulé. » |
...
| Parfum, marque | Supprimer (règle métier existante) | Lignes en `SetNull` avec snapshot ; tarifs et visuels story en cascade. URL lues **avant** le DELETE : `image` et `imageLight` du ou des parfums, logo et variante claire de la marque, planches story. Après le commit, un objet n'est retiré du bucket que si son URL est **dans notre bucket** et que **plus rien ne la référence** (`Perfume`, `Brand`, `SaleLine.imageUrl`, `PerfumeMedia`) : la vignette de l'historique et une fiche dupliquée qui partage l'image restent intactes (un échec de stockage ne rétablit rien : l'objet orphelin est journalisé). Remplacer une image sur une fiche ne supprime pas l'ancienne (`scripts/storage-orphans.ts`). Invalidation vitrine. |
| Visuel story | Retirer (confirmation) | DELETE de la ligne `PerfumeMedia`, puis, après le commit, suppression de l'objet désigné par son URL (stockée par le serveur, jamais reçue du client), seulement si elle est dans notre bucket : un visuel repris avec l'URL d'un autre projet voit sa ligne retirée, jamais son objet. |
...
- **Base sans poche système** (base neuve : tests, préproduction vide — jamais la base reprise, où l'étape 3a la crée) : le serveur crée « Non attribué » dans la transaction du premier geste qui en a besoin, sous l'identifiant fixe `poche-non-attribue` ; deux créations concurrentes butent sur la clé primaire et la seconde retombe sur le rejeu (04 §3.6, §7.4). Aucune lecture ne la crée.
