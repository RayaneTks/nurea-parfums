...
**`.admin-theme` distribue des jetons, il ne peint pas** (**à appliquer**, correction de production `3291428`). La classe est écrite après `@tailwind utilities` : à spécificité égale, ses déclarations passent après toutes les utilitaires. Un `background` porté par `.admin-theme` écrase donc la `bg-*` de chaque nœud qui porte aussi la classe — précisément les nœuds de portail (voile et carte de `ConfirmDialog`, voile et panneau de `Sheet`, palette) : le voile noir translucide devient un aplat gris opaque et la carte du dialogue prend ce même gris. Règle : `.admin-theme` ne déclare que des variables, la couleur de texte, la police et `color-scheme` ; le fond de l'application est peint par une classe dédiée (`.admin-paint`, `background: var(--admin-bg)`) posée seulement sur les conteneurs pleine page (`.admin-app-container`, page de connexion), jamais sur un nœud de portail. Vérifié par `tokens-sync` (aucun `background` dans la règle `.admin-theme`) et par `npm run test:layout` (voile translucide, carte blanche).
...
| `ConfirmDialog` | Confirmation bloquante | `title`, `description`, `confirmLabel` (obligatoire), `cancelLabel`, `tone: danger\|primary` (obligatoire), `onConfirm` (async → busy), `alternative?` (troisième voie : « Masquer plutôt », 06 E19) | Posé en bas d'écran, sous le pouce (feuille d'action iOS) ; focus initial sur « Annuler », jamais sur l'action ; aucune phrase par défaut (l'ancien « Cette action est irréversible » mentait quand l'annulation 5 s existe) ; Radix Dialog, z 90/91 (passe au-dessus des sheets **et** des sheets imbriquées : bande propre, §2.7 — l'ordre de montage des portails ne suffit pas) ; la description **dit la vérité** sur les conséquences (« L'historique est conservé », « Un remboursement est ajouté en face ») : « sans retour possible » seulement là où c'est vrai, « un filet Annuler reste ouvert 5 secondes » là où il existe ; sert aussi les « réserves » du cycle de statuts (tone `primary`). **Règles de production** (`3291428`, `12e2327` ; **à appliquer** pour les trois premières) : `onConfirm` rend une vraie promesse (jamais une fonction qui lance un `startTransition` et rend `void`) — le verrou `busy` et le spinner tiennent jusqu'à la fin de l'écriture, un second tap est sans effet ; **l'échec s'affiche dans la boîte** (message `danger` sous la description, boîte ouverte, boutons réactivés) — jamais en toast, que Radix rend inerte sous la modale ; le corps du texte **défile** sans pousser les boutons hors de l'écran ; le focus initial sur « Annuler » referme le clavier iOS qui recouvrait la boîte |
...
- `UndoProvider` : suppression différée 5 s avec toast « Annuler », survit aux navigations ; un nouveau `scheduleDelete` commit le précédent. *J4* : il passe par le canal de toasts unique (`FeedbackProvider` : un toast à la fois, z 95, et `useConfirm()` pour les réserves de `useAction`) — tout nouveau toast valide la suppression en attente, et l'app qui passe en arrière-plan aussi.
...
| 35 | `.admin-theme` qui peint (écart du 17/09/2026, **à appliquer**) | `background: var(--admin-bg)` sur `.admin-theme`, écrite après les utilitaires : voile et carte des portails repeints en gris opaque (corrigé en production, `3291428` ; **encore présent sur la branche**) | Jetons seulement ; fond peint par `.admin-paint` sur les conteneurs pleine page (§2) | La confirmation était invisible : texte et boutons sur un mur uni |
| 36 | Bandes d'empilement (écart du 17/09/2026, **à appliquer**) | Sheet imbriquée et modale sur 80 / 81, palette 90, toast 95 (branche) | Sheet imbriquée 80/81, modale 90/91, palette 92, toast 100 (§2.7) | L'ordre des portails dans le DOM décidait qui passait devant |
| 37 | Toast portalisé (écart du 17/09/2026, **à appliquer**) | Rendu dans l'arbre de la page (branche) | Portail vers `<body>`, `pointer-events: auto` (§3.1) | Sous une sheet, le filet « Annuler » partait de travers et ne répondait à aucun tap |
| 38 | Erreur de `ConfirmDialog` (écart du 17/09/2026, **à appliquer**) | L'échec de `onConfirm` remonte à l'appelant, qui le toaste (branche) | Message dans la boîte, restée ouverte ; corps défilant (§3.2) | Un toast sous une modale Radix n'est ni lisible par VoiceOver ni tapable, et s'efface en 3 s |
