...
*Mise en œuvre J15.* La liste reste **fermée à cinq GET** : les actions de résultat de la recherche (S17, A16) n'ont pas ouvert de sixième route. « Encaisser xx € » ouvre S02 « Tout encaisser » et réclame les créances du client **et** les poches du moment : les deux voyagent avec la réponse de `GET /api/admin/search` (`CustomerHitDTO.receivables`, `SearchResultsDTO.pockets`), lues seulement quand un résultat client porte une créance — une frappe qui ne trouve personne à encaisser ne paie ni `aEncaisserDetail()` (caché sous `gestion`) ni `activePockets()` (hors cache). Bénéfice d'usage : la sheet s'ouvre **sans aller-retour**, donc sans attente au tap, ce que réclame 06 §4.4 (« la palette se ferme d'abord, puis ouvre la sheet »).
...
- **En-têtes** (`next.config.mjs`, `headers()`) : deux jeux, sur deux sources qui **ne se recouvrent pas** — deux règles qui matchent le même chemin poseraient l'en-tête deux fois, et la valeur double ne vaut rien. Vérifiés par `tests/architecture/securite-headers.test.ts`, qui LIT la configuration.
  - Communs aux deux registres : `Strict-Transport-Security: max-age=63072000; includeSubDomains; preload`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Permissions-Policy` (capteurs, caméra, micro, paiement… tous à vide), `Cross-Origin-Opener-Policy: same-origin`, `X-Permitted-Cross-Domain-Policies: none`, et une **CSP**.
  - Vitrine : `Referrer-Policy: strict-origin-when-cross-origin`, plus `upgrade-insecure-requests`.
  - Gestion (`/admin/:path*`, `/api/admin/:path*`, `/admin-sw.js`) : `Referrer-Policy: same-origin`, `Cross-Origin-Resource-Policy: same-origin`, `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet` — qui **remplace** le `Disallow: /admin` de `app/robots.ts` : ce fichier est public, et y écrire l'adresse du back-office la publiait (audit du 23/09/2026).
- **CSP, et son compromis assumé** : la v1 s'en passait — « nonces sur les scripts inline de Next : coût sans bénéfice pour un opérateur unique ». L'audit de surface d'attaque du 23/09/2026 a relevé l'absence de TOUT en-tête applicatif sur la vitrine, clickjacking compris : la décision est révisée, mais **sans nonce**. Un nonce se pose par requête et interdit le prérendu de la page qui le lit ; la vitrine est statique et porte 207 visuels. `script-src` garde donc `'unsafe-inline'`, ce qui ne couvre PAS l'injection d'un script écrit dans la page. Ce que la politique couvre malgré cela : aucun script d'un autre domaine chargé (`script-src 'self'`), aucune exfiltration vers un autre domaine (`connect-src`), aucun formulaire posté ailleurs (`form-action 'self'`), aucune balise `<base>` (`base-uri 'self'`), aucun plugin (`object-src 'none'`), aucun cadrage (`frame-ancestors 'none'` — le vrai correctif du clickjacking, dont `X-Frame-Options` n'est que le doublon). `'unsafe-eval'` et `ws:` n'apparaissent qu'en développement.
- **Débit** (`src/lib/security/rate-limit.ts`, commun aux deux registres) : fenêtre fixe en mémoire du processus, clé = point d'entrée + condensé de l'adresse. Posée sur `/api/perfume-search` (40/min, plus `s-maxage=60` au bord), sur `submitContactForm` (5 par quart d'heure) et sur `loginAction` (20 par quart d'heure, **avant** toute lecture en base — le verrou de compte de `authWriter` ne voit ni le balayage d'identifiants ni le martèlement d'un compte inexistant). Chaque instance a sa mémoire : c'est un frein, pas un quota ; le volume réparti se traite au pare-feu de l'hébergeur.
- **Formulaire public** (`src/lib/contact/guard.ts`) : champ leurre + temps de saisie minimal, refus **silencieux** (on répond « envoyé » sans envoyer — nommer ce qui a trahi le robot l'aide au tour suivant), longueurs plafonnées, et retours ligne retirés des valeurs qui finissent en en-tête de courriel.
...
Ajouts à l'existant : `headers()` (§8.6 — depuis le 23/09/2026, il couvre AUSSI la vitrine) et `redirects()` (§2.3). `serverExternalPackages: ["@prisma/client"]`, images Supabase, `poweredByHeader: false` conservés.
