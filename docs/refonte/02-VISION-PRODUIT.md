...
**Écart intégré le 17/09/2026.** Ce document a été écrit sur l'ancien `main` local (`47aaad4`) ; la production (`9e0b5d8`) porte onze commits de plus, inventoriés en 01 §3.11. Chaque capacité nouvelle y reçoit ici sa décision (lignes marquées « écart du 17/09/2026 » en §4.1, §4.3, §4.4, §4.5, §4.8, §4.9), et la règle des contenances devient **10 / 50 / 80 ml, 80 par défaut** partout.
...
| Fenêtre de 48 h : une commande livrée quitte le suivi sans être effacée, la compta la garde et son statut y reste modifiable (écart du 17/09/2026, `9a28437`, `3291428`) | **Garder** (forme simplifiée) | Même intention que la production — le suivi montre ce qu'il reste à faire — sans minuterie : la vue « À livrer » ne contient jamais une livrée, le segment « Livrées » les garde toutes (à encaisser d'abord), le statut reste modifiable depuis la fiche. Une recherche traverse toujours les repliés. |
| Recherche commandes et compta étendue : client, contact, notes, lot, parfum, marque, hors catalogue ; plusieurs mots = tous (écart du 17/09/2026, `9a28437`, `521e086`) | **Garder** | On cherche par ce qu'on a en tête (« dior sauvage », un pseudo Snapchat), pas par le champ jugé principal. Plus simple dans la refonte : les noms d'articles sont des colonnes typées (`SaleLine.perfumeName`), plus un JSON. |
| Création : client lié ou libre, lignes catalogue / hors-catalogue, contenances **10 / 50 / 80 ml** (80 par défaut), don, coût DZD + taux, notes du document et note par ligne, date de livraison | **Garder** | Cœur du métier ; le hors-catalogue et le don sont intégrés de bout en bout. Contenances réelles depuis le 10/09/2026 (écart du 17/09/2026) : 30 et 100 désignaient les mêmes flacons, traduits en production ; une contenance hors règle n'est jamais réécrite en silence, elle est demandée. |
...
| Purge « éphémère » : livrées hard-delete à J+1, déclenchée par des GET | **Abandonner** | Perte d'historique et de créances, mouvements orphelins, destruction sur lecture (problème n°6). Remplacée par : rien ne s'efface, les documents soldés se replient. La production l'a elle-même retirée le 10/09/2026 (`3291428`). |
...
| `Order.deliveredAt` + index, `orderListFilterSchema` (mort), `cancelOrderAction` sans appelant | **Abandonner** | Code et champs morts, non reconduits. `Order.deliveredAt` est écrit par la production depuis le 10/09/2026 (trois chemins de livraison, rattrapage des livrées) : la date réelle de livraison est reprise dans `SaleDocument.deliveredAt`, posé par la seule fonction de transition — jamais la date **prévue** (erreur de rattrapage corrigée en production, `9e0b5d8`). |
...
| Recherche client (`?q=`) affichée seulement au-delà de 6 groupes | **Simplifier** | Le champ reste monté tant qu'un filtre est actif (corrige la recherche qui disparaît quand elle réussit — la production l'a corrigé aussi, `521e086`). Portée étendue comme les commandes (§4.1, écart du 17/09/2026). |
| Commandes confirmées rattachées alimentant le total de leur lot dans la compta (écart du 17/09/2026, `d433e18`) | **Simplifier** | Le défaut corrigé — un lot, deux montants selon l'écran — est impossible par construction : la compta ne porte pas de montant d'en-tête de lot (ligne « Liste par sections » ci-dessus), les montants d'un lot se lisent sur sa fiche, par la même fonction partout. |
...
| Section « À rattacher » : tout ce qui n'appartient à aucun lot, commandes **et** ventes, livrées comprises, rattachable ligne par ligne, avec recherche (écart du 17/09/2026, `521e086`) | **Garder** | L'inventaire de ce qu'il reste à ranger n'existait nulle part ; on ne rattache pas ce qu'on ne voit pas. Un envoi terminé se range aussi, pour lui imputer transport et douane. En tête de la liste des lots (06 E05). |
| Rattachement d'une commande en attente ; appartenance affichée quel que soit le statut, montants au périmètre engagé (écart du 17/09/2026, `521e086`, `3707715`) | **Garder** | On sait à quel envoi appartient une commande dès sa création (N9) ; une commande rattachée puis revenue en attente, ou annulée, reste visible et détachable sur la fiche du lot. |
...
| Grille tarifaire `PerfumePricing` par (parfum, contenance 10 / 50 / 80 ml) | **Garder** | Un seul geste d'enregistrement par fiche (plus de double bouton Enregistrer aux effets différents). |
| **Visuels story par parfum** (`PerfumeMedia`) : dépôt de plusieurs planches (HEIC accepté, jamais recadrées), galerie sur la fiche, pastille du nombre de visuels dans la liste (écart du 17/09/2026, `77985aa`) | **Garder** | Le geste « à 22 h, publier la story d'un parfum » : la planche vit là où l'on va déjà vérifier un prix ou un stock, au lieu d'être cherchée dans une pellicule. Distincts du visuel du catalogue, qui reste seul juge de la publication ; jamais lus par la vitrine. Fiche parfum en consultation (06 E16, PC-13). |
| **Récupération par le partage natif** (fichier joint → Snapchat, Photos), sinon téléchargement (écart du 17/09/2026, `77985aa`, `3707715`) | **Garder** | Le seul chemin qui marche sur iPhone en PWA installée ; fermer la feuille de partage ne déclenche rien. |
| Stockage rangé par usage (`stories/<parfum>/`), chemin décidé par le serveur, objets supprimés avec leur visuel ou leur parfum (écart du 17/09/2026, `77985aa`, `b8d015c`, `3707715`) | **Garder** | Un chemin reçu du client finirait dans une suppression d'objet : jamais cru. Rien ne doit rester dans le bucket sans ligne qui le référence (orphelins résiduels : script explicite, 04 §7.3). |
...
- **Garder** : ledger de paiements append-only (généralisé), Trésorerie poches + mouvements signés (le sous-modèle le mieux conçu), `PerfumePricing` (PK composite), snapshots d'historique, `onDelete` SetNull pour l'historique, Decimal partout pour l'argent ; **`PerfumeMedia`** (visuels story, table de production conservée telle quelle, écart du 17/09/2026) ; **contenances réelles 10 / 50 / 80 ml**, contrainte en base, 80 par défaut à la saisie.
...
| Champs jamais lus par la vitrine (`stock`, prix, `Perfume.slug`, `isPrivate`, visuels story `PerfumeMedia`) | — | Libres pour la refonte (décisions en 4.5/4.8) ; toute évolution du contrat serait documentée ici. Un parfum sans visuel de catalogue reste masqué même s'il porte des visuels story. |
