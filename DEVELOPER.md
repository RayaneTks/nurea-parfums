...
- 1) Audit rapide des frictions UX (surtout mobile) avant modifications lourdes.
- 2) Corriger d'abord les points severes: navigation, lisibilite, actions critiques, prevention des erreurs.
- 3) Harmoniser les composants repetes (recherche, boutons d'action, feedbacks).
- 4) Verifier systematiquement:
  - `npm run lint`
  - `npx tsc --noEmit`
- 5) Avant push/PR: controler le message de commit et retirer toute mention IA si elle a ete injectee automatiquement.
